...
func sample(
  ...
  let response = try await client.getNode(
    request: GetNodeRequest()
      .with {
        $0.name =
          "projects/\(projectId)/locations/\(locationId)/privateClouds/\(privateCloudId)/clusters/\(clusterId)/nodes/\(nodeId)"
      }
  )
  ...
  static func main() async throws {
    do {
      ...
      try await sample(
        ...
        privateCloudId: "[placeholder]", clusterId: "[placeholder]", nodeId: "[placeholder]")
    } catch {
      print("Error: \(error)")
    }
  }
