...
func sample(
  ...
  let response = try await client.getSubnet(
    request: GetSubnetRequest()
      .with {
        $0.name =
          "projects/\(projectId)/locations/\(locationId)/privateClouds/\(privateCloudId)/subnets/\(subnetId)"
      }
  )
  ...
  static func main() async throws {
    do {
      ...
      try await sample(
        ...
        privateCloudId: "[placeholder]", subnetId: "[placeholder]")
    } catch {
      print("Error: \(error)")
    }
  }
