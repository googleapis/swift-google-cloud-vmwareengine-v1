...
func sample(
  ...
  let poller = try await client.deleteNetworkPeering(
    withPolling: DeleteNetworkPeeringRequest()
      .with {
        $0.name =
          "projects/\(projectId)/locations/\(locationId)/networkPeerings/\(networkPeeringId)"
      }
  )
