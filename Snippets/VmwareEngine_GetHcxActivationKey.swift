...
func sample(
  ...
  let response = try await client.getHcxActivationKey(
    request: GetHcxActivationKeyRequest()
      .with {
        $0.name =
          "projects/\(projectId)/locations/\(locationId)/privateClouds/\(privateCloudId)/hcxActivationKeys/\(hcxActivationKeyId)"
      }
  )
  ...
  static func main() async throws {
    do {
      ...
      try await sample(
        ...
        privateCloudId: "[placeholder]", hcxActivationKeyId: "[placeholder]")
    } catch {
      print("Error: \(error)")
    }
  }
